...
class CogntioRequest(Request):
    ...
    async def run(self):
        ...
        try:
            headers = self.session._loadlamb_headers
        ...
        headers = {'Authorization': tokens['AuthenticationResult']['IdToken'],'Accept':'application/json; version=1.0'}
        start_time = time.perf_counter()
        try:
            if data:
                data = self.get_choice(data)
                resp = await self.session.request(method_type, path, headers=headers, data=data, timeout=self.timeout)
            elif params:
