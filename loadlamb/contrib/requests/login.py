...
class RemoteLogin(Request):
    ...
    async def run(self):
        ...
        b = await self.session.request('post', self.req_config.get('login_url'), data=get_form_values(a))
        c = await self.session.request('get', b.url)
        self.session.headers.update({'referer': url})
        user = random.choice(self.req_config.get('users'))
        c = await self.session.request('post', b.url, data={'username': user.get('username'),
